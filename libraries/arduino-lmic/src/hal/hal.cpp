...
void lmic_hal_spi_read_2(u1_t cmd, u1_t *addr, size_t addr_len, u1_t *buf, size_t buf_len)
{
    uint32_t spi_freq;
...
void lmic_hal_pollPendingIRQs_helper()
{
    ...
    // if (dio_states[0] != digitalRead(plmic_pins->dio[0]))
    // {
    ...
    u1_t nop = 0x00;
    u1_t buf[2];
    lmic_hal_spi_read_2(0x12, &nop, 1, buf, 2);
...
void lmic_hal_processPendingIRQs()
{
    ...
    ostime_t iTime = interrupt_time[0];
    if (iTime)
    {
        interrupt_time[0] = 0;
...
bit_t lmic_hal_radio_spi_is_busy()
{
    ...
    return digitalRead(plmic_pins->dio[1]) ? true : false;
}
#else
